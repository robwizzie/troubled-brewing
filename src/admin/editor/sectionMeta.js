...
/* A manager entry is either a CollectionManager config (a `table` + fields) or
   a `component` the panel renders as-is. The Reviews control room is the
   second kind: it edits the cached Google library plus a settings block, which
   is not a row-per-item collection. */
export const MANAGER_CONFIGS = {
  ...
  reviews: { title: 'Reviews', component: ReviewsManager, table: 'content_blocks' },
};
