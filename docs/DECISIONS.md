...
The cinematic scene-artwork hero (`immersive_gallery`) becomes the default concept (client loved it on desktop); Gallery Wall and the others stay one click away in Quick Blocks. Its old mobile fallback (flat plaque list) traded away all the whimsy, so mobile was rebuilt as the same room re-hung for phones: each navbar frame becomes a mini framed picture whose art is that frame's **crop out of the same scene file** (pure `background-position` math over `FRAME_LINKS` — zero new assets), hung on a nail + wire with its brass nameplate, plus the gold fox, the chalkboard hours, and the taped specials note. NOTE for already-provisioned DBs: the stored `content_blocks.homepage_concept` still wins — flip it in Quick Blocks or with the one-liner in INTEGRATIONS.md.
