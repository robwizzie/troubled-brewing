...
- **Gallery Wall hero** is a true **salon hang** — an eclectic **masonry** of varied-aspect frames (portrait/wide/oval/tall) with slight rotations, and — per the client's request — **no two moldings alike**, matched to the reference photo of the real wall: grand carved gilt (SVG border-image with corner medallions), thin gold fillet, gold + cream double-mat botanical with a floating keyline, woven gold tapestry, antique carved bronze, a thin brass frame **hung from a chain + nail** (gentle swing), wide flat black, black + wide white mat, a **layered empty second frame**, and gold/black ovals (the gold one mats its art on a cream oval ground). Mats are **white/cream only** (like the shop — no pink frames on the wall). Labels are engraved **brass nameplates** (museum placards — a plain brushed-brass face, no screw heads or arrow). The hero is roomed like the photo: a white **pressed-tin cornice** above, the **black-marble counter band** below, and the **real gold fox-head photo** + a hare tucked among the art. Each frame is a legible **"poster"** (its label in display serif) until a photo is added. Readable on desktop and mobile (4 → 3 → 2 columns); deliberately not a uniform grid. Frame vocabulary lives in `src/lib/frameStyles.js` (legacy style names auto-map).
...
1. **Gallery Wall *landing concept*** (`gallery_wall_hero` section): the homepage hero recreated as a dense arrangement of vintage frames where **each frame is a product/feature** (a latte in the grand gilt frame, sandwiches in the specimen oval, events in the black oval…) and **every frame wears a different molding** (client request — see the frame list under Motion & custom touches). The welcome placard carries the sign's small print: **live open/closed status** (same `HoursToday` as the Warm Storefront concept, computed on the shop's NJ clock) and the owner-editable **"Current Drink Specials"** link (`specials_label`/`specials_link`, default `/menu#specials` → the menu's Specialty tab). Frames animate on hover and link to sections/pages. This is the lead homepage concept — CONFIRMED by the client.
2. **Gallery Wall *page*** (`/gallery-wall`): documents the **actual** framed art on the shop's wall — each real piece gets a photo, title, and a fun backstory. Owner-editable via `gallery_pieces`. The page opens **on the wall itself** (`gallery_wall_room`): the shop's own photograph (`public/images/wall/gallery-wall.jpg`) is the canvas, with a hotspot over each of the nineteen pieces in it. Choosing one does what a visitor would do in the room — the photograph pans and magnifies toward that frame, the rest of the wall drops into shadow behind a spotlight, and a brass-plated card raises the title, the artist, the credits and the story, with arrows to walk the wall piece by piece. It's one `translate(x%, y%) scale(s)` on the layer that carries both the photograph and the hotspots, so nothing can drift off its frame at any zoom, and the pan math is pure percentages of the photograph (`viewFor()` in `src/lib/galleryWall.js`) rather than measured pixels. The magnification is **capped** — the source is 934px wide — and the close-up in the card carries the detail instead: the owner's own straight-on shot of a piece when the collection has one, otherwise a native-resolution crop of the wall. **The hotspot boxes are measured against that exact photograph**; re-shoot the wall and they need re-measuring, the same bargain `wallPieces.js` makes with the homepage scene.
...
1. **Immersive Gallery** *(lead — the default)* — the café-scene artwork IS the homepage: brand lettering painted on the dappled wall, every navbar destination hung as a brass-labeled frame in the scene, a chalkboard with the real live hours, the taped "Today's Special" note, and the newsletter card on the counter. **The pictures inside the scene's frames belong to the scene** — the site draws nothing over them. An earlier pass overlaid the shop's photographs onto each painted frame; the artwork in the room is edited in the artwork itself now, so what the owner exports is exactly what ships, and `igh_scene_image_url` swaps the whole room from the editor (a re-cut scene is an upload, not a commit). Hotspot boxes (`x/y/w/h` in `src/lib/wallPieces.js`) hug each painted molding so the brass nameplate lands on its bottom edge. **On phones the wall re-hangs itself as a real salon hang**: the scene becomes a banner, the counter (live hours board + taped special) comes down directly beneath it, and every destination becomes an actual **framed photograph of the shop** (`public/images/wall/*.jpg`) in its own vintage molding, knocked a degree off true, carrying the **same engraved brass nameplate** as the desktop wall — riding the frame's bottom molding rather than sitting on the picture, which at thumb size costs a real share of the photo. A piece or two hangs **small (84% of the column)** so the hang reads as collected rather than laid out, and every hung thing casts its shadow **down and to the right**, matching the light falling from the upper left of the banner above. It's a CSS **multi-column masonry** (2 columns, 3 from 560px, 1 under 360px) so portraits, landscapes, squares and ovals nest and the columns rebalance at any width; the gold fox-head and brass hare hang among the pictures, and the widest piece on the wall is the **Order Now** sign that closes the hang.
