...
1. **Immersive Gallery** *(lead — the default)* — the cinematic café-scene artwork IS the homepage: brand lettering painted on the dappled wall, every navbar destination hung as a brass-labeled frame in the scene, a chalkboard with the real live hours, the taped "Today's Special" note, and the newsletter card on the counter. **On phones the wall re-hangs itself as a real salon hang**: the scene becomes a banner, the counter (live hours board + taped special) comes down directly beneath it, and every destination becomes an actual **framed photograph of the shop** (`public/images/wall/*.jpg` — the same pictures the Gallery Wall concept hangs) in its own vintage molding, knocked a degree off true, carrying the **same engraved brass nameplate** as the desktop wall — riding the frame's bottom molding rather than sitting on the picture, which at thumb size costs a real share of the photo. A few pieces hang **small (~72% of the column) and pushed off-centre** so the hang reads as collected rather than laid out, and every hung thing casts its shadow **down and to the right**, matching the light falling from the upper left of the banner above. It's a CSS **multi-column masonry** (2 columns, 3 from 640px) so portraits, landscapes, squares and ovals nest and the columns rebalance themselves at any width; the gold fox-head and brass hare hang among the pictures, and the widest piece on the wall is the **Order Now** sign that closes the hang. Same wall, same materials, re-hung for a phone.
