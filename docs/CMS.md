...
| `immersive_gallery_hero` | `{ igh_eyebrow, igh_descriptor, igh_menu_label, igh_hours_label, igh_address, igh_special_label, igh_special_text, igh_mailchimp_action_url, igh_pieces: [{ label, to, img, frame }] }` — `igh_pieces` are **per-frame overrides matched by index** onto `src/lib/wallPieces.js` (see below); everything else — the unbranded scene artwork (`images/wall/immersive-scene.jpg`) is the canvas; branding, frame-link labels (one per navbar destination), chalkboard hours, notes, and signup are live HTML over it (≥1020px); below that the wall re-hangs itself as a salon hang: each link becomes a real framed photograph from `images/wall/` with its brass nameplate, alongside the gold fox + brass hare, and the hours board, taped note and signup come down off the scene | data | `ImmersiveGalleryHero.jsx` |
...
### The Immersive hero's wall

`src/lib/wallPieces.js` holds the wall as data: one entry per destination, carrying BOTH its desktop hotspot geometry (`x/y/w/h`, % of the 1536×1024 scene artwork) and its phone incarnation (molding, aspect, photograph, drawn stand-in). The geometry is tied to the artwork and stays code-owned; the label, link, photo and molding are owner-editable via the hero's `igh_pieces`, merged **by index** by `mergeWallPieces()` so a half-filled row keeps whatever it didn't set. A blank field means "use the built-in"; the sentinel `'-'` in `img` means "hang this one as its drawn stand-in".
